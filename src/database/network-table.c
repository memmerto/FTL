...
int getAliasclientIDfromIP(const char *ipaddr)
{
	...
	if(!FTL_DB_avail())
	{
		logg("getAliasclientIDfromIP(\"%s\") - Database is not available", ipaddr);
		return -1;
	}
...
char *__malloc_like getNameFromIP(const char *ipaddr)
{
	if(!FTL_DB_avail())
	{
...
char *__malloc_like getIfaceFromIP(const char *ipaddr)
{
	if(!FTL_DB_avail())
	{
